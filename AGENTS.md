...
- 对用户给出“可以发给小师妹”的入口前，必须运行 `npm run verify:shareable-delivery -- --url <stable-url> --expected-commit <exact-40-sha> --candidate-root <clean-worktree> --receipt <operator-journey.json>`。稳定域 public health 必须同时证明 `configured=true`、`credential_mode=SERVER_MANAGED`、`access_configured=true`、`authentication_mode=STUDIO_ACCESS_SESSION`、`image_ledger_configured=true`，operator 回执还必须包含 fresh-user `new_draft` 与 `generate_text`；否则旧稿可编辑/导出也只能判 `BLOCKED`。回执还必须绑定一个不同于当前 Production、可直接 promote、曾以 `SERVER_MANAGED + access_required + image ledger` 通过真实生产回读的已知可用 deployment，并给出来源 commit、证据路径与 SHA-256；把已知 Provider 未配置的上一版写成 rollback 同样判 `BLOCKED`。只有机器结果 `HANDOFF_READY` 才能说“可以发给她”；这个工具永远不能签发 `CONSUMER_VALIDATED`，只有小师妹本人在独立设备上的直接反馈才能证明“她已经能用”。`LOCAL_ONLY`、`BLOCKED`、缺回执或任何 Hash/同稿 identity 漂移都必须直说不可交付，禁止用 HTTP 200、Preview Ready、旧 Production、Leon 本机试用或自填 actor 字段升级措辞。
