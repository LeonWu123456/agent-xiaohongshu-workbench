...
test('completed reservation release is same-scope and never deletes results or changes quota limits',async()=>{
 const commands=[];const ledger=createUpstashImageLedger({url:'https://complete-budget.invalid',token:'completed-test-token-12345',fetchImpl:async(_u,o)=>{commands.push(JSON.parse(o.body));return{ok:true,json:async()=>({result:['RELEASED','100']})};}});
 assert.equal(typeof ledger.releaseCompletedPreallocation,'function');await ledger.releaseCompletedPreallocation({runId:'images-123-deadbeef',appScopeId:'xiaoshimei-studio:77777777777777777777777777777777',capacityGeneration:'b'.repeat(64)});
 const lua=commands[0][1];assert.match(lua,/COMPLETE_IMMUTABLE/);assert.match(lua,/COMMITTED/);assert.match(lua,/inventory_count/);assert.match(lua,/cached_response_json/);assert.match(lua,/app_scope/);assert.ok(!/redis\.call\(['"](?:DEL|UNLINK|EXPIRE|PEXPIRE|SET)['"]/.test(lua));
 const before=commands.length;await assert.rejects(()=>ledger.releaseCompletedPreallocation({runId:'wrong-run',appScopeId:'xiaoshimei-studio:77777777777777777777777777777777',capacityGeneration:'b'.repeat(64)}),/INVALID/);assert.equal(commands.length,before);
});

test('released immutable completion remains eligible only for cache-only STEP verification',async()=>{
 const fixture=runtimeLedgerFixture({mode:'STEP'}),g=fixture.envelope.payload.capacity_generation,bytes=fixture.envelope.payload.worst_case_run_bytes;
 const base={app_scope:fixture.appScope,capacity_generation:g,capacity_reservation_bytes:String(bytes),capacity_released:'1',capacity_release_reason:'COMPLETE_IMMUTABLE',status:'COMPLETE',run_json:JSON.stringify({schema:'xiaoshimei.public-image-run.v1',run_id:fixture.runId,status:'COMPLETE',phase:'COMPLETE'}),cached_response_json:JSON.stringify({schema:'xiaoshimei.image-generation-response.v1',status:'COMPLETE',run_id:fixture.runId,error:null})};
 const inspect=async patch=>{const values=Object.entries({...base,...patch}).flat();const x=runtimeLedgerFixture({mode:'STEP',attestation:fixture.signed,runRecord:values});return createUpstashImageLedgerFromEnv(x.env,{fetchImpl:x.fetchImpl}).assertProductionReady({mode:'STEP',runId:x.runId,appScopeId:x.appScope});};
 assert.equal((await inspect({})).cache_only,true);
 for(const patch of [{status:'READY'},{status:'UNKNOWN'},{capacity_release_reason:'manual'},{run_json:'{}'},{cached_response_json:JSON.stringify({status:'PARTIAL'})},{capacity_generation:'a'.repeat(64)},{app_scope:'other'}])await assert.rejects(()=>inspect(patch),/CAPACITY_RESERVATION_INVALID/);
});

test('native Redis releases completed future allocation once and preserves replay, bytes and TTL', {skip:!process.env.XSM_NATIVE_REDIS_ROOT,timeout:60000},async()=>{
 const {spawn,execFile}=await import('node:child_process');const {promisify}=await import('node:util');const {mkdtemp,rm}=await import('node:fs/promises');const exec=promisify(execFile),dir=await mkdtemp('/tmp/xsm-complete-quota-'),socket=dir+'/redis.sock',bin=process.env.XSM_NATIVE_REDIS_ROOT+'/src/';const server=spawn(bin+'redis-server',['--port','0','--unixsocket',socket,'--save','','--appendonly','no','--dir',dir],{stdio:'ignore'});
 const cli=async(...args)=>JSON.parse((await exec(bin+'redis-cli',['-s',socket,'--json','-e',...args.map(String)],{maxBuffer:2000000})).stdout),dict=v=>Array.isArray(v)?Object.fromEntries(Array.from({length:v.length/2},(_,i)=>[v[2*i],v[2*i+1]])):v;
 const scopeId='xiaoshimei-studio:77777777777777777777777777777777',runId='images-123-deadbeef',generation='b'.repeat(64),product='xiaoshimei:image-d37:{xiaoshimei-studio-v2}',scope=product+':scope:'+createHash('sha256').update(scopeId).digest('hex').slice(0,32),root=scope+':run:'+runId,cap=product+':capacity',meta=root+':meta',inventory=root+':inventory',expiry=scope+':expiry',asset=root+':asset:'+('a'.repeat(64)),checkpoint='c'.repeat(64),nonce='d'.repeat(64);let stepKey,deadline;
 const cached=JSON.stringify({schema:'xiaoshimei.image-generation-response.v1',status:'COMPLETE',run_id:runId,error:null,progress:{actual_image_calls:1,completed_image_steps:1,total_image_steps:1},content_package:{body:'Exact paid text'}}),run=JSON.stringify({schema:'xiaoshimei.public-image-run.v1',run_id:runId,status:'COMPLETE',phase:'COMPLETE',actual_image_calls:1,jobs:[{}],next_job_index:1,assets:[{sha256:'a'.repeat(64)}]});
 const ledger=createUpstashImageLedger({url:'https://native-complete-test.invalid',token:'isolated-complete-ledger-fixture',fetchImpl:async(_u,o)=>{try{return{ok:true,json:async()=>({result:await cli(...JSON.parse(o.body))})};}catch(e){return{ok:true,json:async()=>({error:String(e.stderr||e.message).slice(0,300)})};}}});
 const step=overrides=>ledger.reserveStep({runId,appScopeId:scopeId,checkpointPreimageSha256:checkpoint,logicalStepId:'render-job-01',attemptNonce:nonce,ownerToken:'native-owner',accessExpiresAtMs:Date.now()+3600000,...overrides});
 const seed=async(metaPatch={},capPatch={},stepPatch={})=>{
  await cli('FLUSHDB');deadline=Date.now()+8*86400000;
  await cli('HSET',cap,...Object.entries({schema:'xiaoshimei.image-ledger-capacity.v2',capacity_generation:generation,reserved_bytes:'300',live_reservations:'3',unfinalized_inventory:'3',...capPatch}).flat());
  await cli('HSET',meta,...Object.entries({status:'READY',app_scope:scopeId,capacity_generation:generation,capacity_released:'0',capacity_reservation_bytes:'100',reservation_count:'0',inventory_schema:'xiaoshimei.d36-key-inventory.v1',inventory_count:'3',recoverable_until_ms:String(deadline-86400000),physical_expire_at_ms:String(deadline),checkpoint_sha:checkpoint,logical_step_id:'render-job-01'}).flat());
  await cli('SADD',inventory,meta,inventory,asset);await cli('SET',asset,'paid-image-original-bytes');for(const key of [meta,inventory,asset])await cli('PEXPIREAT',key,deadline);await cli('ZADD',expiry,deadline-86400000,meta+'|'+generation+'|100');
  const reserved=await step();assert.equal(reserved.status,'RESERVED');stepKey=root+':step:'+reserved.actionId;
  await cli('HSET',stepKey,...Object.entries({status:'COMMITTED',result_json:cached,...stepPatch}).flat());await cli('HSET',meta,...Object.entries({status:'COMPLETE',run_json:run,cached_response_json:cached,...metaPatch}).flat());
 };
 const snapshot=async()=>({cap:dict(await cli('HGETALL',cap)),meta:dict(await cli('HGETALL',meta)),step:dict(await cli('HGETALL',stepKey)),assets:await cli('GET',asset),inventory:await cli('SMEMBERS',inventory),ttl:await cli('PEXPIRETIME',meta),assetTTL:await cli('PEXPIRETIME',asset),stepTTL:await cli('PEXPIRETIME',stepKey),expiry:await cli('ZRANGE',expiry,0,-1)});
 const release=()=>ledger.releaseCompletedPreallocation({runId,appScopeId:scopeId,capacityGeneration:generation});
 try{let ready=false;for(let n=0;n<100&&!ready;n++){try{ready=await cli('PING')==='PONG';}catch{await new Promise(r=>setTimeout(r,25));}}assert.ok(ready);assert.equal(typeof ledger.releaseCompletedPreallocation,'function');
  for(const state of ['READY','PARTIAL','UNKNOWN','PLANNING','IN_FLIGHT','PLANNER_FAILED']){await seed({status:state});const before=await snapshot();assert.equal((await release()).status,'NOT_ELIGIBLE');assert.deepEqual(await snapshot(),before);}
  for(const patch of [{app_scope:'other'},{run_json:'broken'},{cached_response_json:'{}'},{reservation_count:'2'},{inventory_count:'99'}]){await seed(patch);const before=await snapshot();const result=await release().catch(()=>({status:'THROWN'}));assert.notEqual(result.status,'RELEASED');assert.deepEqual(await snapshot(),before);}
  for(const state of ['IN_FLIGHT','UNKNOWN','LATE_RESULT']){await seed({},{},{status:state});const before=await snapshot();assert.notEqual((await release()).status,'RELEASED');assert.deepEqual(await snapshot(),before);}
  for(const field of ['reserved_bytes','live_reservations','unfinalized_inventory']){await seed({},{[field]:'1.5'});const before=await snapshot();await release().catch(()=>{});assert.deepEqual(await snapshot(),before);}

  // A physical orphan under an otherwise valid COMPLETE run must be rejected
  // before any accounting mutation, not merely detected after the release.
  await seed();const signed=signedRuntimeAttestation({nowMs:Date.now(),appScope:scopeId,restOrigin:'https://native-complete-test.invalid',overrides:{capacity_generation:generation}});
  const readiness=scope+':candidate:'+signed.envelope.payload.candidate_commit+':readiness';await cli('SET',readiness,JSON.stringify(signed.envelope));
  const expected=Object.fromEntries(['database_id_sha256','rest_origin','app_scope','vercel_project_id','vercel_environment','candidate_commit'].map(k=>[k,signed.envelope.payload[k]]));
  const admission=createUpstashImageLedger({url:'https://native-complete-test.invalid',token:'isolated-native-start-fixture',runtimeBinding:{ready:true,publicKey:signed.publicKey,expected},fetchImpl:async(_u,o)=>{try{const args=JSON.parse(o.body);let result=await cli(...args);if(args[0]==='HGETALL'&&result&&!Array.isArray(result))result=Object.entries(result).flat();return{ok:true,json:async()=>({result})};}catch(e){return{ok:true,json:async()=>({error:String(e.stderr||e.message).slice(0,300)})};}}});
  const rogue=root+':asset:orphan-not-in-inventory';await cli('SET',rogue,'unregistered-result-bytes');const priorOrphan=await snapshot();
  await assert.rejects(()=>admission.assertProductionReady({mode:'START',appScopeId:scopeId}),/IMAGE_LEDGER_INVENTORY_UNION_MISMATCH/);
  assert.deepEqual(await snapshot(),priorOrphan,'incomplete union cannot first release counters or expiry');assert.equal(await cli('GET',rogue),'unregistered-result-bytes');
  await cli('DEL',rogue);await admission.assertProductionReady({mode:'START',appScopeId:scopeId});assert.equal(await cli('HGET',cap,'reserved_bytes'),'200');assert.equal(await cli('HGET',meta,'capacity_released'),'1');
  await seed();const before=await snapshot();const result=await Promise.all([release(),release()]);assert.deepEqual(result.map(x=>x.status).sort(),['ALREADY_RELEASED','RELEASED']);const after=await snapshot();assert.equal(after.cap.reserved_bytes,'200');assert.equal(after.cap.live_reservations,'2');assert.equal(after.cap.unfinalized_inventory,'2');assert.equal(after.meta.capacity_released,'1');assert.equal(after.meta.capacity_release_reason,'COMPLETE_IMMUTABLE');for(const key of ['run_json','cached_response_json','recoverable_until_ms','physical_expire_at_ms','reservation_count','capacity_reservation_bytes'])assert.equal(after.meta[key],before.meta[key]);for(const key of ['step','assets','inventory','ttl','assetTTL','stepTTL'])assert.deepEqual(after[key],before[key]);assert.deepEqual(after.expiry,[]);
